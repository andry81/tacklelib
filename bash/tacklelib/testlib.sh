...
[[ -n "$BASH" && (-z "$BASH_LINENO" || BASH_LINENO[0] -gt 0) ]] && (( ! SOURCE_TACKLELIB_TESTLIB_SH )) || return 0 || exit 0 # exit to avoid continue if the return can not be called
...
# special variable to a direct inclusion in the `tkl_testmodule_run_test` function
...
function tkl_test_init()
{
  ...
  TestScriptLogDir="${TestSessionId}-${TestScriptEntry}"
...
function tkl_testmodule_run_test()
{
  ...
  local TestStdoutDeclare="$2" # stdout reference generated by the test, lines is separated by `:` character
  shift 2
  local TestFuncArgs=("$@")
