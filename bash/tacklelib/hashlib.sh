...
function tkl_crc32_bsd()
{
  ...
  local str="$1"
  local crc=$(( ${2:-0} & 0xFFFFFFFF ))

  # table from bsd libkrnl
  local tkl__crc32_table__bsd=(
  ...
  local len=${#str}
  for (( i=0; i < len; i++ )); do
    tkl_char_to_ubyte "${str:i:1}"
    crc=$(( tkl__crc32_table__bsd[(crc ^ RETURN_VALUE) & 0xFF] ^ (crc >> 8) ))
  done

  RETURN_VALUE=$(( crc ^ 0xFFFFFFFF ))
}

function tkl_crc32_gnu()
{
  ...
  for (( i=0; i < len; i++ )); do
    tkl_char_to_ubyte "${str:i:1}"
    crc=$(( tkl__crc32_table__gnu[(crc >> 24) ^ RETURN_VALUE] ^ ((crc << 8) & 0xFFFFFFFF) ))
  done
  for (( i=len; i; i >>= 8 )); do
    crc=$(( tkl__crc32_table__gnu[((crc >> 24) ^ i) & 0xFF] ^ ((crc << 8) & 0xFFFFFFFF) ))
  done
...
function tkl_hash_func_body_as_token()
{
  local FuncName="$1"
  ...
  tkl_crc32 "$FuncBody" tkl_crc32_gnu || return 1
...
# The same as tkl_hash_func_body_as_token but hashes whole function including function signature.
function tkl_hash_func_as_token()
{
  local FuncName="$1"
