...
if [[ -n "$BASH" && (-z "$BASH_LINENO" || BASH_LINENO[0] -gt 0) && "$TACKLELIB_SCRIPTS_TOOLS_PROJECTLIB_SH" != "$BASH_SOURCE_DIR" ]]; then

source '/bin/bash_entry' || exit $?
tkl_include '$TACKLELIB_BASH_SCRIPTS_ROOT/buildlib.sh' || tkl_abort_include

TACKLELIB_SCRIPTS_TOOLS_PROJECTLIB_SH="$BASH_SOURCE_DIR" # including guard

function GenerateSrc()
{
  local CONFIG_FILE_IN="$PROJECT_CONFIG_SCRIPTS_ROOT/01_generate_src/${BASH_SOURCE_FILE_NAME%[.]*}.in"
  ...
  local CONFIG_FILE_IN="$PROJECT_CONFIG_SCRIPTS_ROOT/01/${BASH_SOURCE_FILE_NAME%[.]*}.deps.${BASH_SOURCE_FILE_NAME##*[.]}.in"
...
function Configure()
{
  ...
  tkl_include "$TACKLELIB_BASH_SCRIPTS_ROOT/tools/set_vars_from_locked_file_pair.sh" || tkl_abort_include
  ...
  tkl_include "$PROJECT_SCRIPTS_ROOT/__init__/__init2__.sh" || tkl_abort_include
...
function CheckConfigVersion()
{
  ...
  for flag in "${flag_args[@]}"; do
    if [[ "$flag" == "-optional_system_file_instance" ]]; then
      flag_optional_system_file_instance=1
    elif [[ "$flag" == "-optional_user_file_instance" ]]; then
  ...
  if [[ -n "$VARS_SYSTEM_FILE_IN" && ! -e "$VARS_SYSTEM_FILE_IN" ]]; then
    echo "$0: error: VARS_SYSTEM_FILE_IN does not exist: \`$VARS_SYSTEM_FILE_IN\`." >&2
    return 10
  fi

  if [[ flag_optional_system_file_instance -eq 0 && -n "$VARS_SYSTEM_FILE_IN" && ! -e "$VARS_SYSTEM_FILE" ]]; then
    echo "$0: error: VARS_SYSTEM_FILE does not exist: \`$VARS_SYSTEM_FILE\`." >&2
    return 11
  fi

  if [ -n "$VARS_USER_FILE_IN" && ! -e "$VARS_USER_FILE_IN" ]]; then
    echo "$0: error: VARS_USER_FILE_IN does not exist: \`$VARS_USER_FILE_IN\`." >&2
    return 20
  fi

  if [[ flag_optional_user_file_instance -eq 0 && -n "$VARS_USER_FILE_IN" && ! -e "$VARS_USER_FILE" ]]; then
  ...
  local VARS_FILE_IN_VER_LINE='.'
  local VARS_SYSTEM_FILE='.'
  ...
  if [[ -e "$VARS_SYSTEM_FILE_IN" && -e "$VARS_SYSTEM_FILE" ]]; then
  ...
  VARS_FILE_IN_VER_LINE='.'
  VARS_SYSTEM_FILE='.'

  if [[ -e "$VARS_USER_FILE_IN" && -e "$VARS_USER_FILE" ]]; then
    read -r VARS_FILE_IN_VER_LINE < "$VARS_USER_FILE_IN"
    read -r VARS_FILE_VER_LINE < "$VARS_SYSTEM_FILE"
