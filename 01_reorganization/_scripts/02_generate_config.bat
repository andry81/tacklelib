...
for %%i in (PROJECT_ROOT PROJECT_LOG_ROOT PROJECT_CONFIG_ROOT CONTOOLS_ROOT PROJECT_GEN_ROOT) do (
  if not defined %%i (
...
if not exist "%PROJECT_LOG_ROOT%" ( mkdir "%PROJECT_LOG_ROOT%" || exit /b )
...
:MAIN
set "CMDLINE_SYSTEM_FILE_IN=%PROJECT_CONFIG_ROOT%/_scripts/02/%~n0.system%~x0.in"
set "CMDLINE_USER_FILE_IN=%PROJECT_CONFIG_ROOT%/_scripts/02/%~n0.user%~x0.in"
...
set "CONFIG_FILE_IN=%PROJECT_CONFIG_ROOT%/_scripts/02/%~n0.deps%~x0.in"
