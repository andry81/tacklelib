...
namespace utility {
    ...
    FORCE_INLINE std::wstring string_format(size_t string_reserve, const std::wstring fmt_str, ...)
    {
        size_t str_len = (std::max)(fmt_str.size(), string_reserve);
        std::wstring str;
        ...
        while (true) {
            ...
            const int final_n = vswprintf(const_cast<wchar_t *>(str.data()), str_len, fmt_str.c_str(), ap);
